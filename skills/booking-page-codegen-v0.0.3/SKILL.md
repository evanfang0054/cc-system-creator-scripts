...
1. **需求理解专家分析**：调用 booking-requirement-analyzer 代理进行专业的需求分析和代码理解
2. **数据集成专家设计**：调用 booking-data-integration-expert 代理进行API集成和数据层架构设计
3. **UI交互专家规划**：调用 booking-ui-interaction-expert 代理进行组件分析和交互逻辑设计
4. **代码整合专家实现**：调用 booking-code-integrator 代理进行代码生成和PRD验收
5. **测试验证专家检查**：调用 booking-test-validator 代理进行全面测试和质量验证
...
1. 彻底阅读并执行 `.claude/skills/booking-page-codegen/step1.md` 中的指令，该步骤会调用 `booking-requirement-analyzer` 代理。
...
1. 彻底阅读并执行 `.claude/skills/booking-page-codegen/step2.md` 中的指令，该步骤会调用 `booking-data-integration-expert` 代理。
...
1. 彻底阅读并执行 `.claude/skills/booking-page-codegen/step3.md` 中的指令，该步骤会调用 `booking-ui-interaction-expert` 代理。
...
1. 彻底阅读并执行 `.claude/skills/booking-page-codegen/step4.md` 中的指令，该步骤会调用 `booking-code-integrator` 代理。
...
1. 彻底阅读并执行 `.claude/skills/booking-page-codegen/step5.md` 中的指令，该步骤会调用 `booking-test-validator` 代理。

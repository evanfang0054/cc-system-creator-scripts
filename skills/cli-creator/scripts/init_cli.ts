...
function collectDependencies(config: CliConfig): { dependencies: string[], devDependencies: string[] } {
  ...
  // ✅ 新增: 交互式提示 (advanced 模板)
  if (config.template === 'advanced') {
    dependencies.push(...DEPENDENCY_MAP.prompts.inquirer);
  }
  ...
  // ✅ 新增: 输出格式化 (standard 模板)
  if (config.template !== 'minimal') {
    dependencies.push(...DEPENDENCY_MAP.formatting.cliTable);
  }
...
async function generateLibFiles(config: CliConfig, srcDir: string): Promise<void> {
  ...
  // ✅ 非 minimal 模板生成 validation.ts (参数验证工具)
  if (config.template !== 'minimal') {
  ...
  // ✅ advanced 模板生成 prompts.ts (交互式提示)
  if (config.template === 'advanced') {
  ...
  // ✅ 非 minimal 模板生成 exit-codes.ts (退出码标准化)
  if (config.template !== 'minimal') {
    const exitCodesTemplate = await fs.readFile(
      path.join(__dirname, 'templates/exit-codes.ts'),
      'utf-8'
    );
    await fs.writeFile(path.join(libDir, 'exit-codes.ts'), exitCodesTemplate);
  }

  // ✅ advanced 模板生成 config-loader.ts (配置文件层级)
  ...
  // ✅ standard 模板生成 update-check.ts (版本检查)
  if (config.template !== 'minimal') {
    const updateCheckTemplate = await fs.readFile(
      path.join(__dirname, 'templates/update-check.ts'),
      'utf-8'
    );
    await fs.writeFile(path.join(libDir, 'update-check.ts'), updateCheckTemplate);
  }

  // ✅ standard 模板生成 formatters.ts (输出格式化)
  if (config.template !== 'minimal') {
    const formattersTemplate = await fs.readFile(
      path.join(__dirname, 'templates/formatters.ts'),
      'utf-8'
    );
    await fs.writeFile(path.join(libDir, 'formatters.ts'), formattersTemplate);
  }

  // ✅ standard 模板生成 summary.ts (操作摘要)
  if (config.template !== 'minimal') {
    const summaryTemplate = await fs.readFile(
      path.join(__dirname, 'templates/summary.ts'),
      'utf-8'
    );
    await fs.writeFile(path.join(libDir, 'summary.ts'), summaryTemplate);
  }

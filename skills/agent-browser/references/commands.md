...
agent-browser snapshot -s "#main" # 限定范围到 CSS 选择器
```
...
使用 `-b`/`--base64` 或 `--stdin` 以确保可靠执行。Shell 转义中的嵌套引号和特殊字符容易出错。
...
agent-browser state load auth.json    # 恢复已保存的状态
```
...
agent-browser profiler stop trace.json    # 停止并保存性能分析
```

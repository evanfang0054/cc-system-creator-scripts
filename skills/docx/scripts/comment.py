...
用法：
    python comment.py unpacked/ 0 "批注文本"
    python comment.py unpacked/ 1 "回复文本" --parent 0

文本应预先进行 XML 转义（例如 &amp; 表示 &，&#x2019; 表示智能引号）。

运行后，需在 document.xml 中添加标记：
...
添加到 document.xml（标记必须是 w:p 的直接子元素，绝不能放在 w:r 内部）：
...
在父批注 {pid} 的标记内部嵌套标记（标记必须是 w:p 的直接子元素，绝不能放在 w:r 内部）：
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="向 DOCX 文档添加批注")
    p.add_argument("unpacked_dir", help="解压后的 DOCX 目录")
    p.add_argument("comment_id", type=int, help="批注 ID（必须唯一）")
    p.add_argument("text", help="批注文本")
    p.add_argument("--author", default="Claude", help="作者名称")
    p.add_argument("--initials", default="C", help="作者缩写")
    p.add_argument("--parent", type=int, help="父批注 ID（用于回复）")
    args = p.parse_args()

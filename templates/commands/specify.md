...
用户在触发消息中输入 `/speckit.specify` 之后的文本**就是**功能描述。假设你始终可以在本次对话中使用它，即使下面 `{ARGS}` 按字面显示。除非用户提供空命令，否则不要要求用户重复输入。

基于该功能描述，执行以下操作：

1. **为分支生成一个简洁的短名称**（2-4个单词）：
   - 分析功能描述并提取最有意义的关键词
   - 创建一个2-4个单词的短名称，能够捕捉功能的本质
   - 尽可能使用"动作-名词"格式（例如："add-user-auth"、"fix-payment-bug"）
   - 保留技术术语和缩写（OAuth2、API、JWT等）
   - 保持简洁但描述性强，能够一目了然地理解功能
   - 示例：
...
2. **在创建新分支之前检查现有分支**：
   ...
   d. 使用计算出的编号和短名称运行脚本 `{SCRIPT}`：
      ...
      - Bash示例：`{SCRIPT} --json --number 5 --short-name "user-auth" "Add user authentication"`
      - PowerShell示例：`{SCRIPT} -Json -Number 5 -ShortName "user-auth" "Add user authentication"`
   ...
   **重要提示**：
   ...
   - 每个功能只能运行一次此脚本
   - JSON在终端中作为输出提供 - 始终参考它以获取你正在寻找的实际内容
...
5. 使用模板结构将规范写入SPEC_FILE，用从功能描述（参数）派生的具体细节替换占位符，同时保持部分顺序和标题。
...
6. **规范质量验证**：编写初始规范后，根据质量标准进行验证：
   ...
   c. **处理验证结果**：
      ...
      - **如果有项失败（不包括 [需要澄清]）**：
        1. 列出失败项和具体问题
        2. 更新规范以解决每个问题
        3. 重新运行验证，直到所有项都通过（最多3次迭代）
        4. 如果3次迭代后仍然失败，在检查清单备注中记录剩余问题并警告用户

      - **如果存在 [需要澄清] 标记**：
        1. 从规范中提取所有 [需要澄清: ...] 标记
        2. **限制检查**：如果存在超过3个标记，仅保留3个最关键的（按范围/安全/UX影响）其余的做出合理猜测
        3. 对于每个需要澄清的项目（最多3个），以以下格式向用户展示选项：
...
7. 报告完成情况，包括分支名称、规范文件路径、检查清单结果以及下一阶段（`/speckit.clarify` 或 `/speckit.plan`）的就绪状态。
...
5. **像测试人员一样思考**：每个模糊的需求都应该使"可测试且明确"的检查清单项失败
...
- "API响应时间在200ms以下"（太技术化，使用"用户立即看到结果"）
